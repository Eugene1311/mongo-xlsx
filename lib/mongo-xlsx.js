...
exports.mongoData2Xlsx = function(monogData, mongoModel, options, callback) {
  ...
  return xlsxRW.writeXlsx(xlsxData, options, function(err, data) {
    callback(err, data);
  });
};
...
exports.mongoData2XlsxMultiPage = function(excelDataArray, sheetNamesArray, options, callback) {
  ...
  return xlsxRW.writeXlsxMultiPage(excelDataArray, sheetNamesArray, options, function(err, data) {
    callback(err, data);
  });
};
...
exports.mongoData2XlsxData = function(mongoData, mongoModel) {
  ...
  for (var dataIndex = 0; dataIndex < mongoData.length; dataIndex+=1) {
    ...
    for (index = 0; index < mongoModel.length; index+=1) {
      ...
      if (parse.type === 'date' && !Number.isNaN(new Date(aCell).valueOf())) {
  ...
  for (var index = 0; index < excelData.length; index+=1) {
    if (index === 0) {
      ...
      for (var rIndex = 0; rIndex < aRow.length; rIndex+=1) {
        ...
        // If column has no header, then generate static string: _no_header_at_col_X
        if (!accessString) {
          accessString = '_no_header_at_col_' + rIndex;
        }
        // Header may be not string if is special type. e.g., date
        else if (typeof accessString !== 'string') {
          accessString += '';
        }
